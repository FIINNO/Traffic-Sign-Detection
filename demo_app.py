...
def load_model():
    ...
    model.load_state_dict(torch.load("checkpoint_V8_best_v2.pth", map_location=lambda storage, loc: storage))
    return model
...
st.markdown(
    ...
    /* Ensure headers and other elements also use black text */
    h1, h2, h3, h4, h5, h6, p, div, span {
        color: black !important;
    }

    /* Optional: Change sidebar background and text */
    .css-1d391kg {
...
if uploaded_file:
    ...
    image, preds, padding, inference_time = predict(img_np, model)
    ...
    with col2:
        ...
        for sign_info in detected_signs:
            st.image(sign_info["crop"], use_container_width=True, clamp=True)
            st.markdown(f"##### Class: {sign_info['cls']}")
            st.markdown(f"##### Confidence: {sign_info['conf']:.2f}")
